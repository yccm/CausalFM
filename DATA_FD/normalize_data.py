...
def normalize_and_save_datasets(input_pattern, output_dir, save_scalers=True):
    """
    Normalize all front-door datasets and save them to disk once.
    ...
    (continuous) mediator `m`:
      ...
      - Outcomes:       a single StandardScaler fitted on [y0, y1], then applied
                        ...
                        in the normalized space (and `ate` as its mean, if present).
      - Mediator:       a single StandardScaler fitted on the available mediator
                        columns ([m0, m1] when present, else `mediator`), then
                        applied to `mediator`, `m0`, `m1`.
    ...
    for idx, file in enumerate(tqdm(csv_files, desc="Normalizing datasets")):
        ...
        # Recalculate ITE (and ATE) in normalized space
        df_normalized[ite_col] = df_normalized[y1_col] - df_normalized[y0_col]
        if ate_col in df.columns:
            df_normalized[ate_col] = df_normalized[ite_col].mean()

        # Normalize the (continuous) mediator with a shared scaler.
        # Fit on the potential mediators [m0, m1] when available, mirroring the
        # way the outcome scaler is fitted on [y0, y1]; otherwise fit on `mediator`.
        has_m0 = m0_col in df.columns
        has_m1 = m1_col in df.columns
        if has_m0 and has_m1:
            m_combined = np.concatenate([
                df[m0_col].values.reshape(-1, 1),
                df[m1_col].values.reshape(-1, 1)
            ])
            m_scaler.fit(m_combined)
        else:
            m_scaler.fit(df[m_col].values.reshape(-1, 1))

        df_normalized[m_col] = m_scaler.transform(df[m_col].values.reshape(-1, 1)).flatten()
        if has_m0:
            df_normalized[m0_col] = m_scaler.transform(df[m0_col].values.reshape(-1, 1)).flatten()
        if has_m1:
            df_normalized[m1_col] = m_scaler.transform(df[m1_col].values.reshape(-1, 1)).flatten()
...
class CausalDatasetFast(Dataset):
    """
    Faster dataset class that loads pre-normalized front-door data.
    ...
    def __init__(self, csv_files, transform=None):
        ...
        for idx, file in enumerate(self.csv_files):
            ...
            sequence = {
                ...
                'm': torch.FloatTensor(df[m_col].values).unsqueeze(1),
...
if __name__ == "__main__":
    ...
    normalize_and_save_datasets(
        input_pattern="DATA_FD/frontdoor/*.csv",
        output_dir="DATA_FD/frontdoor_normalized",
        save_scalers=True
    )
